...
def get_bucket_kms_key(bucket: str) -> str | None:
    """
    Return the KMS key ARN configured for SSE-KMS on a bucket, or None.

    Returns the key ARN for both customer-managed and AWS-managed (aws/s3) keys.
    Returns None if the bucket uses SSE-S3 (AES256) or has no default encryption.

    Note: AWS-managed keys (aws/s3) do not require explicit IAM kms:Decrypt/kms:Encrypt
    grants — callers should check for this before adding IAM permissions.

    :param bucket: S3 bucket name.
    :return: KMS key ARN string, or None if no KMS encryption is configured.
    ...
    try:
        ...
        if code in ("NoSuchBucket", "404"):
            raise RuntimeError(f"Bucket {bucket!r} does not exist — create it before running this command") from e
        if code == "AccessDenied":
            raise RuntimeError(
                f"Access denied reading encryption config for {bucket!r} — "
                "cannot auto-detect KMS key. Pass it explicitly via --source-kms-key or --dest-kms-key."
            ) from e
