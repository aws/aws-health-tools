...
from s3_batch_replication.types import KmsKeyArn, S3BucketArn, S3Uri
...
def _build_permissions(source_bucket: str, dest_bucket: str, source_kms_key: str | None, dest_kms_key: str | None, manifest_bucket: str | None = None, report_bucket: str | None = None) -> dict:
    statements = [
        ...
        {
            "Effect": "Allow",
            "Action": ["s3:ReplicateObject", "s3:ReplicateTags", "s3:ObjectOwnerOverrideToBucketOwner"],
    ...
    if report_bucket:
        report_bucket_name = report_bucket.split(":::")[-1]
        statements.append({
            "Effect": "Allow",
            "Action": "s3:PutObject",
            "Resource": f"arn:aws:s3:::{report_bucket_name}/*",
        })

    if source_kms_key and not source_kms_key.endswith("aws/s3"):
        source_kms_region = source_kms_key.split(":")[3]
        statements.append({
            "Effect": "Allow",
            "Action": "kms:Decrypt",
            "Resource": source_kms_key,
            "Condition": {"StringEquals": {"kms:ViaService": f"s3.{source_kms_region}.amazonaws.com"}},
        })

    if dest_kms_key and not dest_kms_key.endswith("aws/s3"):
        dest_kms_region = dest_kms_key.split(":")[3]
        statements.append({
            "Effect": "Allow",
            "Action": ["kms:Encrypt", "kms:GenerateDataKey"],
            "Resource": dest_kms_key,
            "Condition": {"StringEquals": {"kms:ViaService": f"s3.{dest_kms_region}.amazonaws.com"}},
        })
...
@click.option("--report-bucket", type=S3BucketArn(), default=None, shell_complete=complete_buckets, help="S3 bucket (name or ARN) for job completion reports — grants the role s3:PutObject write access")
...
def setup_iam_role(ctx: click.Context, source_bucket: str, destination_bucket: str, role_name: str | None, source_kms_key: str | None, dest_kms_key: str | None, manifest: str | None, report_bucket: str | None, force: bool) -> None:
    ...
    role_name = role_name or f"s3-batch-replication-{hashlib.sha256(source_bucket.encode()).hexdigest()[:43]}"
    ...
    policy_name = f"S3BatchReplication-{hashlib.sha256(destination_bucket.encode()).hexdigest()[:16]}"
