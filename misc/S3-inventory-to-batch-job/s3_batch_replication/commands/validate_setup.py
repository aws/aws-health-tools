...
@click.option("--no-check-destination-policy", is_flag=True, default=False, help="Skip destination bucket policy check")
...
def validate_setup(
    ...
    no_check_destination_policy: bool,
    no_check_kms: bool,
) -> None:
    ...
    if not no_check_destination_policy:
        if not destination_bucket:
            _result(ctx, "Destination bucket policy", False, "destination bucket not specified")
            failed = True
        else:
            failed |= _check_bucket_policy_action(ctx, s3, destination_bucket, "Destination bucket policy", "s3:ReplicateObject")
