...
def _count_rows(bucket: str, key: str) -> int:
...
@click.option("--dry-run", is_flag=True, default=False, help="Show how many jobs would be created without uploading anything")
@click.option("--count-objects", is_flag=True, default=False, help="Download all inventory files to get exact object counts (slow — downloads all CSV data, use with --dry-run)")
@click.pass_context
def split(ctx: click.Context, manifest: str | None, objects_per_job: int, objects_per_manifest_file: int | None, output: str | None, max_objects: int | None, failure_threshold: float, continue_after_failure: bool, dry_run: bool, count_objects: bool) -> None:
    ...
    if not manifest:
        raise click.UsageError("--manifest is required when not chained after setup-iam-role")
    ...
    if dry_run:
        if count_objects:
            echo(ctx, f"Counting objects in {len(manifest_obj.files)} inventory file(s)...")
            manifest_bucket, _ = parse_s3_uri(manifest)
            file_counts: dict[str, int] = {}
            with ThreadPoolExecutor(max_workers=min(os.cpu_count() // 2 or 1, len(manifest_obj.files))) as pool:
                futures = {pool.submit(_count_rows, manifest_bucket, f.key): f.key for f in manifest_obj.files}
                for future in as_completed(futures):
                    key = futures[future]
                    count = future.result()
                    file_counts[key] = count
                    logger.debug("  %s: %d rows", key, count)
            total_exact = sum(file_counts.values())
            echo(ctx, f"Dry run: {total_parts} job(s) would be created")
            echo(ctx, f"  Total inventory files: {len(manifest_obj.files)}  Exact total objects: {total_exact:,}")
            for i, partition in enumerate(partition_files(manifest_obj.files, files_per_job)):
                job_count = sum(file_counts[f.key] for f in partition)
                echo(ctx, f"  Job {i + 1}: {len(partition)} file(s) ({job_count:,} objects)")
        else:
            total_estimated = len(manifest_obj.files) * objects_per_manifest_file
            echo(ctx, f"Dry run: {total_parts} job(s) would be created")
            echo(ctx, f"  Total inventory files: {len(manifest_obj.files)}  Estimated total objects: ~{total_estimated:,} (based on --objects-per-manifest-file={objects_per_manifest_file:,})")
            for i, partition in enumerate(partition_files(manifest_obj.files, files_per_job)):
                echo(ctx, f"  Job {i + 1}: {len(partition)} file(s) (~{len(partition) * objects_per_manifest_file:,} objects estimated)")
        return
